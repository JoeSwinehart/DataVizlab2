...
```{r raw data}
# I deleted this code because (a) it's generally considered impolite to
# install things on other people's computer without first asking permission, 
# and (b) it actually wasn't doing what you wanted anyway, which is to say
# it should have first checked to see if the package was installed, and then
# installed it if not, but instead it was checking to see if the package was
# needed, and then installing it if it was. So, best to just keep install
# commands out of scripts, but you can put them in and comment them out if you
# have an audience that may not know how to install packages.
...
ggplot(data = trends_clean, aes(x = date, y = Ranking, color = Hurricane)) +
  geom_line() + 
  scale_color_discrete(name = "Hurricane", 
...
# Made some edits to get your geom_text to work the way I think you were trying 
# to get it to work

# just a slightly different way to create the data frame (not necessarily better)
hurricane_landfall <- data.frame(
  date = as.Date(c('2017-08-25', '2017-09-10', NA, '2017-09-20')), 
  Hurricane = c('hurricane_harvey_us', 'hurricane_irma_us', 
               'hurricane_jose_us', 'hurricane_maria_us'),
  label_loc = c(50, 90, NA, 30)
)
...
ggplot(trends_clean, aes(date, Ranking, color = Hurricane)) +
  ...
  geom_label(aes(label = paste0("Landfall\n", format(hurricane_landfall$date, "%b %m")), 
                 y = label_loc),
             data = hurricane_landfall) + #<----- #This freaking line won't work ignore it
  theme_minimal() +
...
Replicate the “National cable news networks” plot from the story using the tv_states data. Don’t worry about all the labels. Try to match the colors and **don't** worry/obsess about getting them exact.

```{r tv states viz}

# this would have been quite a bit less code if you moved it to a longer format
# first so you had a "state" or "place" column (or whatever you want to call it)
...
#Filling in ribbons second  
  geom_ribbon(aes(x = date, y = florida, ymin = 0, ymax = florida), 
              fill = "#F55285", color = "white", alpha = 0.98) +   
  geom_ribbon(aes(x = date, y = texas, ymin = 0, ymax = texas), 
              fill = "#ED723C", color = "white", alpha = 0.98) + 
  geom_ribbon(aes(x = date, y = puerto_rico, ymin = 0, ymax = puerto_rico), 
              fill = "#40C1C9", color = "white", alpha = 0.98) +  
#Identifying incidents (e.g., hurricane landfalls)
  geom_text(aes(x = as.Date("2017-08-25"), y = 3.1, label = "Harvey\nLandfall"), 
            size = 4, color = "grey") +
  geom_vline(xintercept = as.Date("2017-08-25"), 
             linetype = "dotted", color= "grey", size = 0.75) +
  geom_text(aes(x = as.Date("2017-09-07"), y = 3.3, label = "Irma\nLandfall"), 
            size = 4, color = "grey") +
  geom_vline(xintercept = as.Date("2017-09-07"), 
             linetype = "dotted", color= "grey", size = 0.75) +
  geom_text(aes(x = as.Date("2017-09-18"), y = 2.1, label = "Maria\nLandfall"), 
            size = 4, color = "grey") +
  geom_vline(xintercept = as.Date("2017-09-18"), 
             linetype = "dotted", 
             color = "grey", 
             size = 0.75) +
...
# I stopped editing - it looks great but isn't super efficient. Reformatting
# the data first would have helped.
```
...
genderplot2$sex <- factor(genderplot2$sex, 
                          levels = ordered_gender$sex)

#Plot
ggplot(genderplot2, aes(x = sex, y = appearances, fill = sex)) +
  geom_jitter(aes(color = sex, width = .25), 
              show.legend = FALSE, size = .65, stat = "identity") +
  scale_colour_viridis_d(direction = -1) +
  labs(title = "DC and Marvel Comic Characters by Gender Identification",
       x = "",
       y = "") +
  annotate("text", x = 1, y = 3300, label = ordered_gender$appearances[1], 
           size = 4, color = "black")+
  annotate("text", x = 2, y = 1900, label = ordered_gender$appearances[2], 
           size = 4, color = "black")+ 
  annotate("text", x = 3, y = 600, label = ordered_gender$appearances[3], 
           size = 4, color = "black")+ 
  annotate("text", x = 4, y = 400, label = ordered_gender$appearances[4], 
           size = 4, color = "black")+ 
  annotate("text", x = 5, y = 350, label = ordered_gender$appearances[5], 
           size = 4, color = "black")+ 
  annotate("text", x = 6, y = 150, label = ordered_gender$appearances[6], 
           size = 4, color = "black")+ 
  theme_gray() +
  theme(
