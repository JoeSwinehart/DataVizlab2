...
genderplot2 <- comic_characters %>%
  ...
  #group_by(sex) %>% 
  #count() %>% 
  #mutate(rename("Agender" == 'Agender Characters'))
  #I had group_by() and count() to be able to specify gender ID and sort, but could not remember how to sort.


#Plot
ggplot(genderplot2, aes(x = sex, y = appearances, fill = sex)) +
  geom_jitter(aes(color = sex, width = .25,), show.legend = FALSE, size = .65) +
...
#To Do:  
# - Reorder sex by descending count
# - Font and size for y-axis label and title
# - Change sex so it does not include the word "Characters"
